...
Eigen::Array2i  alphabeta_pruning(t_node root, uint8_t depth) {
    BitBoard    moves = get_player_open_adjacent_positions(root.p1, root.p2);
    ...
    return ((Eigen::Array2i){best_pos / 19, best_pos % 19}); // y, x
}
...
uint32_t        max(t_node node, uint32_t alpha, uint32_t beta, uint8_t current_depth, uint8_t const& max_depth) {
    ...
    BitBoard                    moves = get_player_open_adjacent_positions(node.p1, node.p2) ^ node.p1_forbidden;
    uint32_t                    score;
    uint32_t                    max;

    uint8_t cpc = (node.current_player_id == 1 ? node.p1_pairs_captured : node.p2_pairs_captured);
    if (current_depth == max_depth || check_end(node.p1, node.p2, node.p1_pairs_captured, node.p2_pairs_captured)) {
    ...
    for (uint16_t i = 0; i < 361; i++) {    
        if (moves[i] != 0) {
            ...
            score = min(node, alpha, beta, current_depth+1, max_depth);
...
uint32_t        min(t_node node, uint32_t alpha, uint32_t beta, uint8_t current_depth, uint8_t const& max_depth) {
    node.p2_forbidden = forbidden_detector(node.p2, node.p1);
    BitBoard                    moves = get_player_open_adjacent_positions(node.p2, node.p1) ^ node.p2_forbidden;
    uint32_t                    score;
    uint32_t                    min;

    uint8_t cpc = (node.current_player_id == 1 ? node.p1_pairs_captured : node.p2_pairs_captured);
    if (current_depth == max_depth || check_end(node.p2, node.p1, node.p2_pairs_captured, node.p1_pairs_captured)) {
        node.p1_forbidden = forbidden_detector(node.p1, node.p2);
        return score_function(node);
    }
    min = 0xFFFF;
    for (uint16_t i = 0; i < 361; i++) {
        if (moves[i] != 0) {
            ...
            score = max(node, alpha, beta, current_depth+1, max_depth);
...
uint32_t        score_function(t_node node) {
    ...
    for (uint16_t j = 0; j < 361; j++) {
        for (uint16_t i = 0; i < 11; i++) {
            score += (boards[i][j] >> (64 - j & 0x3F)) * BitBoard::patterns[i].value;
        }
    }
    return (score);
}
