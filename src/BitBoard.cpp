...
void    BitBoard::write(const int x, const int y) {
    const int    n = (19 * y + x);
    this->values[n >> 6] |= (0x8000000000000000 >> (n & 0x3F));
}

void    BitBoard::remove(const int x, const int y) {
    const int    n = (19 * y + x);
    this->values[n >> 6] &= ~(0x8000000000000000 >> (n & 0x3F));
}

void    BitBoard::write(const int i) {
    this->values[i >> 6] |= (0x8000000000000000 >> (i & 0x3F));
}

void    BitBoard::remove(const int i) {
    this->values[i >> 6] &= ~(0x8000000000000000 >> (i & 0x3F));
}
...
bool    BitBoard::check_bit(const int i) const {
    return ((this->values[i >> 6] & (0x8000000000000000 >> (i & 0x3F))) == 0 ? false : true);
}

bool    BitBoard::check_bit(const int x, const int y) const {
