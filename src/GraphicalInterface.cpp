...
void    GraphicalInterface::update_end_game(Player *player) {
    if (this->_game_engine->check_end(player->get_pair_captured()) == 1) { // TODO : implement draw condition (check_end() == 2)
...
void    GraphicalInterface::_render_secondary_viewport(void) {
    ...
    SDL_RenderFillRect(this->_renderer, &rect);
    rect = { 0, (int32_t)(180 * this->_res_ratio)+1, this->_secondary_viewport.w, 1 };
    SDL_RenderFillRect(this->_renderer, &rect);
    rect = { 0, (int32_t)(270 * this->_res_ratio)+1, this->_secondary_viewport.w, 1 };
    SDL_RenderFillRect(this->_renderer, &rect);
